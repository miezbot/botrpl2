...
module.exports.run = (bot, message, args) => {
  ...
  if (!['pc','xbl','psn'].includes(args[0])) return message.channel.send('**Masukkan platform: `rplfortnite [ pc | xbl | psn ] <username>`**');
  // We also need the username, which would be args[1] & on...
  if (!args[1]) return message.channel.send('**Masukkan username: `rplfortnite [ pc | xbl | psn ] <username>`**');
